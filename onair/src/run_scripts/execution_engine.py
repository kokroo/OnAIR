...
class ExecutionEngine:
    ...
    def parse_configs(self, config_filepath):
        ...
        self.telemetryFiles = config["DEFAULT"][
            "TelemetryFiles"
        ]  # Vehicle telemetry data
        try:
            self.benchmarkFilePath = config["DEFAULT"]["BenchmarkFilePath"]
            self.benchmarkFiles = config["DEFAULT"][
                "BenchmarkFiles"
            ]  # Vehicle telemetry data
    ...
    def parse_data(
        ...
        dataFilePath,
        metadataFilePath,
        subsystems_breakdown=False,
    ):
        parser = importlib.import_module(
            "onair.data_handling.parsers." + parser_file_name
        )
        parser_class = getattr(
            parser, parser_name
        )  # This could be simplified if the parsers all extend a parser class... but this works for now
        tm_data_path = os.environ["RUN_PATH"] + dataFilePath
        tm_metadata_path = os.environ["RUN_PATH"] + metadataFilePath
        parsed_data = parser_class(
            tm_data_path,
            tm_metadata_path,
            self.telemetryFiles,
            self.metaFiles,
    ...
    def save_results(self, save_name):
        ...
        save_path = (
            os.environ["ONAIR_SAVE_PATH"] + "/saved/" + save_name + "_" + complete_time
        )
        os.mkdir(save_path)
